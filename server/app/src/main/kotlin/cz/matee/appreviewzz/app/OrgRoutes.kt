        ...
        route("/api") {
            ...
            requireSession(auth) {
                ...
                post("/invitations/accept") {
                    ...
                    val organization =
                        io {
                            orgs.acceptInvitation(call.authenticatedUser.account, SecretPayload(request.token))
                        }
...
private suspend fun ApplicationCall.orgContext(
    organizations: OrganizationRepository,
    memberships: MembershipRepository,
): OrgContext {
...
private fun ApplicationCall.userIdParam(): UserId =
    runCatching { UserId(Uuid.parse(parameters["userId"].orEmpty())) }
        .getOrElse { throw ConsoleException(ConsoleFailure.NOT_FOUND, "Takový člen tu není") }
